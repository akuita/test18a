...
import { useRecordEmployeeCheckIn } from '@services/attendance'
...
const AttendanceSystemPage: React.FC = () => {
  ...
  const { mutate: recordCheckIn } = useRecordEmployeeCheckIn({
    onSuccess: () => {
      // Handle successful check-in (e.g., show a message to the user)
    },
    onError: (error) => {
  ...
  const handleCheckIn = () => {
    // Assuming employee_id is available, replace `1` with the actual employee ID
    recordCheckIn({ employee_id: 1 });
  }
        ...
        <div className={styles.content}>
          ...
          <div className={styles.buttonGroup}>
            <Button
              ...
              className={styles.checkInButton} onClick={handleCheckIn}
            />
            <Button
